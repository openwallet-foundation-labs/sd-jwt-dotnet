...
## A pragmatic adoption path: a 90-day pilot a risk team can approve

Start with high-volume, low-regret "guidance mode" journeys (education, explanations, ranges, next steps). Avoid regulated personal advice journeys until operating boundaries are explicit.

### Weeks 1-2: Choose a bounded use case and define the disclosure policy

Pick one or two intents with clear evidence requirements (e.g., claim status and next steps; contribution verification). Define:
- required claims
- optional claims
- forbidden claims
- fallback behaviors when verified context is unavailable

### Weeks 3-6: Implement the Verified Context Gate

Deliver:
- OID4VP presentation request per intent
- SD-JWT VC verification
- issuer trust list management (fund-issued, administrator-issued, insurer-issued)
- evidence receipt storage (hash-bound, policy versioned)
- correlation IDs across channel -> VCG -> AI -> downstream systems

### Weeks 7-10: Add AI orchestration with safety controls

Deliver:
- prompts constrained to minimal verified claims only
- grounding against approved knowledge sources (fund rules, PDS summaries, operational SOP)
- Prompt Shields / Guardrails integrated as second-line controls
- HITL workflows for edge cases and risk thresholds

### Weeks 11-13: Prove governance and operational readiness

Deliver:
- red-team tests (prompt injection, data exfiltration, role misuse)
- audit trail validation end-to-end (receipt -> AI input -> output -> decision)
- board-level risk and controls pack (mapping to CPS 230 / CPS 234 / SPS 220 / APP principles)

KPIs:
- containment/deflection rate for the selected intents
- reduction in average handling time for assisted channels
- unverified model calls in "verified journeys" (must be zero)
- blocked policy violations (proof the gate works)
