...
| **Scope**            | Issuer DI setup, key configuration, `SdJwtVcIssuer` and `SdJwtVcPayload` usage, HAIP policy enforcement, and under-the-hood mechanics. Out of scope: verification (see [Verifying Presentations](verifying-presentations.md)), revocation (see [Managing Revocation](managing-revocation.md)). |
