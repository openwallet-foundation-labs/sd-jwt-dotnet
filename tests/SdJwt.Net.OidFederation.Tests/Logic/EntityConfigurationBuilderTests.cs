...
public class EntityConfigurationBuilderTests
{
    ...
    public EntityConfigurationBuilderTests()
    {
    ...
    [Fact]
    public void WithSigningKey_WithUnsupportedAlgorithm_ShouldThrow()
    {
        // Arrange
        var builder = EntityConfigurationBuilder.Create("https://issuer.example.com");

        // Act & Assert
        var act = () => builder.WithSigningKey(_signingKey, "HS256");
        act.Should().Throw<ArgumentException>()
           .WithMessage("Unsupported signing algorithm: HS256*");
    }
